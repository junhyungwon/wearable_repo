...
static int ti81xxfb_allocate_fbs(struct ti81xxfb_device *fbdev)
{
	...
	if(def_vram) {
        if(ti81xxfb_parse_vram_param(def_vram, TI81XX_FB_NUM, vrams)) {
            dev_err(fbdev->dev, " failed to parse vram parameters: %s\n", def_vram);
            memset(vrams, 0, sizeof(vrams));
        } else {
            printk(" vram %s\n", def_vram);
        }
    } else {    //# phoong : fixed vram size(6M) of fb (default)
        vrams[0] = 0x400000;    //# 4M (1920x1080x2, 0x3F4800)
        vrams[1] = 0x100000;    //# 1M ( 800x 480x2,  0xBB800)
        vrams[2] = 0x100000;    //# 1M ( 720x 480x2,  0xA8C00)
        printk(" vram default: fb0(4M), fb1(1M), fb2(1M)\n");
    }
