...
{
	return __raw_readl(GPIO0_BASE + GPIO_DATAIN)&0xF;
}
...
void fitt_gpio_init(void)
{
    ...
    gpio_direction_input(B_REV_P4);            //# (0, 4)
    gpio_direction_input(REC_SWITCH);      	   //# (0, 6)
    gpio_direction_input(ACCEL_IRQ);           //# (0, 16)
    gpio_direction_output(SERDES_3_PWR, 0);	   //# (0, 17)
	gpio_direction_output(SERDES_4_PWR, 0);	   //# (0, 18)
    gpio_direction_input(USER_2_LED);    	   //# (0, 19)
    gpio_direction_output(SERDES_2_PWR, 0);	   //# (0, 28)

    /* GPIO BANK_1 */
    gpio_direction_input(MMC_CD);               //# (1, 7)
    gpio_direction_output(ETH_STATUS_LED, 0);  	//# (1, 8) default off
    gpio_direction_input(EXT_DETECT);           //# (1, 13)
    gpio_direction_input(USB_GPS_DETECT);       //# (1, 14)
	gpio_direction_output(NAND_WP, 1);	   	   	//# (1, 25)
	gpio_direction_output(RSTOUT_EN, 1);    	//# (1, 27) //# eth, aic3101x reset.

	/* GPIO BANK_2 */
	gpio_direction_input(S_GPS_PWR);       		//# (2, 4) default off
	gpio_direction_input(REC_LED);          	//# (2, 5)
	gpio_direction_output(BACKUP_LED, 0);      	//# (2, 6) default off

	gpio_direction_input(MCU_BSL_RTS);			//# (2, 21)
	gpio_direction_output(MICOM_HOLD, 1);		//# (2, 26) //# default power hold

	/* GPIO BANK_3 */
	gpio_direction_input(MCU_BSL_DTR);			//# (3, 10)
	gpio_direction_output(SERDES_1_PWR, 0);  	//# (3, 14), disable
	gpio_direction_input(ETH_IRQ);         		//# (3, 19)

	__led_init(REC_LED, STATUS_LED_OFF);
}
...
static void sata_pll_config()
{
    ...
    /* setup sata1 pll - for external 100Mhz */
...
#include "mux.h"
	...
	/*
	 * 0x000C:XXXX --> slow/input/pull-down
	 * 0x0006:XXXX --> input/pull-up
	...
	__raw_writel(0x00040080, 0x48140820); /* GP0[2] */
	__raw_writel(0x00040080, 0x48140824); /* GP0[3] */
	__raw_writel(0x00040080, 0x48140828); /* GP0[4] */

	__raw_writel(0x00050080, 0x48140830); /* GP0[6]:input enable(pull-dis) */
	__raw_writel(0x00060080, 0x481409A8); /* GP1[13]:input enable(pull-up) */
	__raw_writel(0x00050080, 0x481409AC); /* GP1[14]:input enable(pull-dis) */
	...
	__raw_writel(0x00060080, 0x48140B70); /* GP3[13]:VOUT[1]_G_Y_YC[8].GP3[13], pincntl221*/

	/* pull-up/down disable input enable  */
	__raw_writel(0x00050080, 0x48140A28); /* GP2[4] */
}
