...
static const char *rtmp_endpoint  = "rtmp://3.35.55.222:1935/live2/myStream/key";

// libuv. fixme : separated file.
pthread_t *thread;
uv_loop_t *loop;
...
void timer_cb (uv_timer_t* timer, int status) {
    fprintf(stderr, "* timer check rtmp status %d.\n", rtmp_ready);
    if (rtmp_ready == 0) {
        int r = uv_async_send(async_rtmp_connect);
    }
}

void async_cb_rtmp_connect(uv_async_t* async) {
    rtmp_ready = 0;
...
void uv_thread(void* context){
    fprintf(stderr, "* libuv will start event loop\n");
    ...
    r = uv_async_init(loop, async_rtmp_connect, async_cb_rtmp_connect);

    // fixme : keep the loop alive. timer every 10s
    timer = malloc(sizeof(uv_timer_t));
    uv_timer_init(loop, timer);
    uv_timer_start(timer, timer_cb, 1000* 10, 1000* 10);

    //Start loop
    uv_run(loop, UV_RUN_DEFAULT);

    fprintf(stderr, "* libuv loop exited\n");
    uv_loop_close(loop);
