...
int util_set_net_info(const char *name) 
{
    ...
    if (!strcmp(name, "eth0"))
    {
        ...
        {
            util_disk_info(&disk_info, "/mmc/DCIM");
            inet.type   = app_set->net_info.type;
            inet.state  = dev_net_link_detect(name);

            if (!inet.state) 
            {
                dev_net_link_up(name);
                sleep(1);
            ...
            if (inet.state) 
            {
                dev_get_net_info(name, &inet);
                if ((!strcmp(inet.ip, "0.0.0.0")) || (strcmp(inet.ip, app_set->net_info.eth_ipaddr) !=0 )) 
                {
                    strcpy(inet.ip, app_set->net_info.eth_ipaddr);
                }

                if ((!strcmp(inet.mask, "0.0.0.0")) || (strcmp(inet.mask, app_set->net_info.eth_netmask) != 0)) 
                {
                    strcpy(inet.mask, app_set->net_info.eth_netmask);
                }

                if ((!strcmp(inet.gate, "0.0.0.0")) || (strcmp(inet.gate, app_set->net_info.eth_gateway) != 0)) 
                {
                    strcpy(inet.gate, app_set->net_info.eth_gateway);
                }

                dev_set_net_info(name, &inet);
                sleep(1);
            }
        }
        else  // DHCP
        {
            f = popen("/sbin/udhcpc -t 5 -n -i eth0", "w");
// -t retries -n exit with failure if lease is not immediately obtained
            if (f != NULL)      
            {
                pclose(f);       
                dev_get_net_info(name, &inet);
                sprintf(app_set->net_info.eth_ipaddr, "%s", inet.ip);
                sprintf(app_set->net_info.eth_gateway, "%s", inet.gate);
                sprintf(app_set->net_info.eth_netmask, "%s", inet.mask);
    ...
    else if (!strcmp(name, "eth1"))
    {
        if(app_set->net_info.type == NET_TYPE_STATIC)
        {
            ...
            {
                dev_get_net_info(name, &inet);
                if ((!strcmp(inet.ip, "0.0.0.0")) || (strcmp(inet.ip, app_set->net_info.wlan_ipaddr) !=0 )) 
                {
                    strcpy(inet.ip, app_set->net_info.wlan_ipaddr);
                }

                if ((!strcmp(inet.mask, "0.0.0.0")) || (strcmp(inet.mask, app_set->net_info.wlan_netmask) != 0)) 
                {
                    strcpy(inet.mask, app_set->net_info.wlan_netmask);
                }

                if ((!strcmp(inet.gate, "0.0.0.0")) || (strcmp(inet.gate, app_set->net_info.wlan_gateway) != 0)) 
                {
                    strcpy(inet.gate, app_set->net_info.wlan_gateway);
                }
        ...
        {
            f = popen("/sbin/udhcpc -t 5 -n -i eth1", "w");
// -t retries -n exit with failure if lease is not immediately obtained
            if (f != NULL)      
            {
                pclose(f);       
    ...
    else 
    {
    	...
    	/* Wi-Fi wlan0, */
            ...
            if (fd > 0) 
            {
	         ...
	         //memset(&sin, 0, sizeof(struct sockaddr));
	            ...
	            sin.sin_addr.s_addr = inet_addr(app_cfg->wmode.wlan_ipaddr);

	            memcpy(&ifr.ifr_addr, &sin, sizeof(struct sockaddr));
	            ioctl(fd, SIOCSIFADDR, &ifr);

	            sin.sin_addr.s_addr = inet_addr(app_cfg->wmode.wlan_netmask);
	            memcpy(&ifr.ifr_addr, &sin, sizeof(struct sockaddr));
	            ret = ioctl(fd, SIOCSIFNETMASK, &ifr);
	            if (ret < 0)
	                dprintf("netmask: Invalid argument\n");

	            dev_set_net_if_gate(ifr.ifr_name, app_cfg->wmode.wlan_gateway);
	            close(fd);
	        } 
		}
		else
    	...
    	/* Wi-Fi wlan0, */
                ...
                if (fd > 0) 
                {
	    	        ...
	    	        //memset(&sin, 0, sizeof(struct sockaddr));
	                sin.sin_family = AF_INET;
	                sin.sin_addr.s_addr = inet_addr(app_set->net_info.wlan_ipaddr);

	                memcpy(&ifr.ifr_addr, &sin, sizeof(struct sockaddr));
	                ioctl(fd, SIOCSIFADDR, &ifr);

	                sin.sin_addr.s_addr = inet_addr(app_set->net_info.wlan_netmask);
	                memcpy(&ifr.ifr_addr, &sin, sizeof(struct sockaddr));
	                ret = ioctl(fd, SIOCSIFNETMASK, &ifr);
	                if (ret < 0)
	                    dprintf("netmask: Invalid argument\n");

	                dev_set_net_if_gate(ifr.ifr_name, app_set->net_info.wlan_gateway);
	                close(fd);
	            } else
	    	        ret = -1;
            }
            ...
            {
                sprintf(buff, "/sbin/udhcpc -t 5 -n -i %s",name) ;
// -t retries -n exit with failure if lease is not immediately obtained
                f = popen(buff, "w");
                if (f != NULL)      
                {
