#include "WW_H264VideoSource_FIFO.h"
#include <stdio.h>
#ifdef WIN32
#include <windows.h>
#else
...
WW_H264VideoSource_FIFO::WW_H264VideoSource_FIFO(UsageEnvironment & env) : 
FramedSource(env),
m_pToken(0),
m_pFrameBuffer(0),
m_hFifo(0)
{
	m_hFifo = open(FIFO_NAME,O_RDONLY);
        printf("[MEDIA SERVER] open fifo result = [%d]n",m_hFifo);
	if(m_hFifo == -1)
	{
		return;
	}
	...
	{
		printf("[MEDIA SERVER] error malloc data buffer failedn");
		return;
	}
	memset(m_pFrameBuffer,0,REV_BUF_SIZE);
}

WW_H264VideoSource_FIFO::~WW_H264VideoSource_FIFO(void)
{
	if(m_hFifo)
	{
		::close(m_hFifo);
...
void WW_H264VideoSource_FIFO::doGetNextFrame()
{
	// fps
...
unsigned int WW_H264VideoSource_FIFO::maxFrameSize() const
{
	return 1024*200;
}

void WW_H264VideoSource_FIFO::getNextFrame(void * ptr)
{
	((WW_H264VideoSource_FIFO *)ptr)->GetFrameData();
}

void WW_H264VideoSource_FIFO::GetFrameData()
{
