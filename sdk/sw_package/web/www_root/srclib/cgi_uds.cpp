		...
		int data_len)
{
	...
	switch(cmd) {
		...
		case UDS_SET_NETWORK_CONFIG:
			...
			break;
		case UDS_GET_NETWORK_CONFIG2:
			{
				// 1. write command
				sprintf(wbuf, "%s", STR_MSG_GET_NETWORK_CONFIG2);
				ret = write(cs, wbuf, sizeof wbuf);
				CGI_DBG("Sent CMD:%s\n",wbuf);

				// wait response and read sizeof T_CGI_NETWORK_CONFIG bytes
				ret = read(cs, data, sizeof(T_CGI_NETWORK_CONFIG2));
				CGI_DBG("Read, size:%d\n", ret);
			...
			break;
		case UDS_SET_NETWORK_CONFIG2:
			{
				// 1. write command
				sprintf(wbuf, "%s", STR_MSG_SET_NETWORK_CONFIG2);
				ret = write(cs, wbuf, sizeof wbuf);
				CGI_DBG("Sent CMD:%s\n", wbuf);
				...
				// 3. write id/password info
				ret = write(cs, data, sizeof(T_CGI_NETWORK_CONFIG2));
				CGI_DBG("Sent Data, written len = %d\n", ret);
